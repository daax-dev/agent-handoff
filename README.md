...
| Primitive | Behavior |
|-----------|----------|
| `agent({ prompt, schema?, agent?, model?, array?, timeoutMs? })` | Spawn one sub-agent (via the CLI adapter layer); returns the schema-validated value, or raw text when no schema. `timeoutMs` overrides the per-agent timeout. Retries 3× before surfacing failure. |
