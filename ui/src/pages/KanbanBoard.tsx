      ...
      {visibleColumns.length === 0 ? (
        ...
        <div className="flex gap-3 overflow-x-auto p-4 flex-1 items-start">
          {visibleColumns.map((status) => (
            <div
              key={status}
              className={`flex-shrink-0 w-[220px] flex flex-col rounded-lg border bg-card border-t-2 ${COLUMN_BORDER[status] ?? "border-t-border"}`}
            >
              <div className="flex items-center gap-2 px-3 py-2.5 border-b sticky top-0 bg-card rounded-t-lg z-10">
              ...
              <div className="p-2 flex flex-col gap-2 overflow-y-auto max-h-[calc(100vh-200px)]">
